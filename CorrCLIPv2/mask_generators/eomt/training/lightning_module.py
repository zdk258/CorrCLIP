...
except ImportError:  # older torchmetrics lacks these; only train/val metrics need them, not inference
    PanopticQuality = MeanAveragePrecision = None
try:
...
except ImportError:  # older torchmetrics lacks this module; only val metrics need it, not inference
    _prepocess_inputs = _Color = _get_color_areas = _calculate_iou = None
import wandb
...
class LightningModule(lightning.LightningModule):
    ...
    def init_metrics_instance(self, num_blocks):
        if MeanAveragePrecision is None:   # old torchmetrics: skip val metrics (not needed for inference)
            self.metrics = nn.ModuleList()
            return
        self.metrics = nn.ModuleList(
    ...
    def init_metrics_panoptic(self, thing_classes, stuff_classes, num_blocks):
        if PanopticQuality is None:        # old torchmetrics: skip val metrics (not needed for inference)
            self.metrics = nn.ModuleList()
            return
        self.metrics = nn.ModuleList(
            [
