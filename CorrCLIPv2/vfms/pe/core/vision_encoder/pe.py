...
from pe.core.vision_encoder.rope import Rope2D
from pe.core.vision_encoder.config import PEConfig, PETextConfig, PE_VISION_CONFIG, PE_TEXT_CONFIG, fetch_pe_checkpoint
    ...
    def forward_features(
        ...
        if not ovss:
            ...
            for blk in self.transformer.resblocks:
                x = blk(x)
                if norm:
                    feats.append(self.ln_post(x))
                else:
                    feats.append(x)
            return feats, (grid_h, grid_w)

    def forward(self, x: torch.Tensor, dino_feats=None, feat_shape=None, masks=None, ovss=False, **kwargs):
        if not ovss:
            ...
            feats, token_size = self.forward_features(x, norm=True, ovss=True, **kwargs)
            x = feats[-1]
            batch, _, _ = x.shape

            q = self.attn_pool.probe.repeat((batch, 1, 1)).to(x.dtype)
            # x = self.attn_pool.attn(q, x, x, need_weights=False)[0]
            # x = x + self.attn_pool.mlp(self.attn_pool.layernorm(x))

            # masks = F.interpolate(masks.float(), size=token_size, mode='nearest').int().squeeze(1)
            # masks = masks[0]
            # region_masks = [idx == masks for idx in torch.unique(masks)]
            # q = q.repeat(1, len(region_masks), 1)
            # region_masks = torch.stack(region_masks, dim=0)
            # attn_mask = region_masks.view(region_masks.shape[0], -1)
            # attn_mask = ~attn_mask
            # cls_attn_mask = torch.zeros((len(region_masks), 1)).bool().to(attn_mask.device)
            # attn_mask = torch.cat([cls_attn_mask, attn_mask], dim=1)
            #
            # mct = self.attn_pool.attn(q, x, x, need_weights=False, attn_mask=attn_mask)[0]
            # mct = mct[0]
            # expand_masks = F.interpolate(region_masks.unsqueeze(1).to(mct.dtype), size=feat_shape, mode='nearest').flatten(1, -1)
            # x_semantic = expand_masks.T @ mct
            # x = x_semantic.unsqueeze(0)
            #
            # x = x + self.attn_pool.mlp(self.attn_pool.layernorm(x))
            ...
            for batch_idx in range(similarity.shape[0]):
                region_masks = [(idx == masks[batch_idx]).view(-1) for idx in torch.unique(masks[batch_idx])]
            ...
            attn = self.attn_pool.attn
            embed_dim = attn.embed_dim
            v_proj_weight = attn.in_proj_weight.narrow(0, 2 * embed_dim, embed_dim)
            v_proj_bias = attn.in_proj_bias.narrow(0, 2 * embed_dim, embed_dim)
            v = F.linear(x, v_proj_weight, v_proj_bias)
            v = v[:, 1:]
            v = v.transpose(1,2).unflatten(sizes=token_size, dim=-1)
            v = F.interpolate(v, size=feat_shape, mode='bilinear')
            v = v.flatten(2, 3).transpose(1, 2)
            ...
            # v = F.interpolate(v, size=feat_shape, mode='bilinear')
            # v = v.flatten(2, 3).transpose(1, 2)
            # similarity = torch.einsum("b m c, b n c -> b m n", v, v)
            # equal_idx = equal_idx.unsqueeze(0).expand_as(similarity)
            # similarity[~equal_idx] = float('-inf')
            # attn_weights = F.softmax(similarity, dim=-1)
            # x = torch.bmm(attn_weights, v)
            # x = x.transpose(0, 1).contiguous().view(-1, bsz, embed_dim).transpose(0, 1)
            # x = attn.out_proj(x)
            # x = x + self.attn_pool.mlp(self.attn_pool.layernorm(x))
...
class CLIP(TextTransformer):
    ...
    def encode_image(self, image, dino_feats=None, feat_shape=None, instance_masks=None, ovss=False, normalize: bool = False):
        x = self.visual(image, dino_feats, feat_shape, instance_masks, ovss)
