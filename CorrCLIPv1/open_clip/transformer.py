...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dino_feats, feat_shape, masks):
        B, nc, h, w = x.shape
        ...
        ALPHA, BETA = 1.0, 0.5  # Coefficients that balance two additional branches
        ...
        dino_feats = dino_feats.reshape(B, *feat_shape, -1)
        ...
        masks = F.interpolate(masks.float().unsqueeze(1), size=token_size, mode='nearest').int().squeeze(1)
        masks = masks[0]
        region_masks = [idx == masks for idx in torch.unique(masks)]
        mct = x[:1].repeat(len(region_masks), 1, 1)
        num_mct = mct.shape[0]
        x = torch.cat([mct, x], dim=0)
        attn_mask = torch.zeros((x.shape[0], x.shape[0]), device=x.device).bool()
        attn_mask[num_mct:, num_mct:] = True 
        attn_mask[:num_mct, num_mct] = True
        region_masks = torch.stack(region_masks, dim=0)
        attn_mask[:num_mct, num_mct + 1:] = region_masks.view(region_masks.shape[0], -1)  
        attn_mask = ~attn_mask
        ...
        for idx, blk in enumerate(self.transformer.resblocks[:-1]):
            ...
            if num_layers - 2 - NUM_SPATIAL <= idx <= num_layers - 3:
                feats_spatial.append(x[num_mct:])

        for blk in self.transformer.resblocks[-1:]:
            # Main Branch and Spatial Branch
            x_main_spatial = torch.cat([x[num_mct:], torch.stack(feats_spatial).mean(0)], dim=1)
            attn_weights = attn_weights.expand(B * 2, -1 ,-1)
            ...
            v = F.linear(blk.ln_1(x_main_spatial), v_proj_weight, v_proj_bias)  # (L, B, C)
            v = v.permute(1, 2, 0)[:, :, 1:]  # B, C, L
            v = v.unflatten(dim=2, sizes=token_size)  # B, C, H, W
            ...
            # Semantic Branch
            mct = blk(x, attn_mask=attn_mask)[:num_mct, 0]
            ...
            x_semantic = x_semantic.unsqueeze(1)
